...
class RGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_rels,skip_dim,skip=False,dropout=0.0,num_bases=-1,use_cuda=False, bias=None,
        ...
        if self.Fusion:
            self.Fusion_dim = 16
            self.Fusion_weights = []
            self.Fusion_weight1 = nn.Linear(self.Fusion_dim,self.out_feat, bias=True)
            self.Fusion_weight2 = nn.Linear(self.Fusion_dim,self.out_feat, bias=True)
            self.Fusion_weight3 = nn.Linear(self.Fusion_dim,self.out_feat, bias=True)
